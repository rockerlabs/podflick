...
struct ITunesDBWriter {
    ...
    private func buildPlaylistMhyp(
        ...
        for (index, item) in items.enumerated() {
            mhips += try cloneItem(
                in: donor, trackID: item.id, itemID: base + 1 + UInt32(index),
    ...
    /// Clones the donor's last mhip, swapping in the per-item ids. `position`
    /// sets the child mhod100 order value explicitly (createPlaylist passes a
    /// distinct index per item); when nil the donor's value + 2 is kept — the
    /// single-item add() behavior, where no sibling can collide.
    private func cloneItem(in donorList: ITunesDB.Playlist, trackID: UInt32,
                           itemID: UInt32, trackDBID: UInt64,
                           itemDBID: UInt64, timestamp: UInt32,
                           position: UInt32? = nil) throws -> Data {
        guard let donor = donorList.items.last else {
